...
Gmail uses its pullaway for one task, not many, blah blah blah.
...
* **Flipboard's flipping.** Please steal this. Nobody else seems to. Flipboard is one of the best iOS apps because it's so natural, and it's so natural because it's so responsive. Flipping hides latency by rate limiting the user to a few articles at a time. There's no scroll jerking. More apps should creatively hide UI lag rather than throw everything in a table view.
